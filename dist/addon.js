...
// EPG support
const service_1 = require("./epg/service");
const nameMap_1 = require("./epg/nameMap");
...
function vdbg(...args) { if (process.env.VAVOO_DEBUG !== '0') {
    try {
        console.log('[VAVOO]', ...args);
    }
    catch { }
} }
// Optional logos map loaded from disk: key `${countryId}:${cleanName.toLowerCase()}` -> logo URL
const LOGOS_FILE = path_1.default.join(__dirname, 'logos-map.json');
let logosMap = {};
function readLogosFromDisk() {
    try {
        const raw = fs_1.default.readFileSync(LOGOS_FILE, 'utf8');
...
const CATEGORIES_FILE = path_1.default.join(__dirname, 'categories-map.json');
let categoriesMap = {};
function readCategoriesFromDisk() {
    try {
        const raw = fs_1.default.readFileSync(CATEGORIES_FILE, 'utf8');
        const j = JSON.parse(raw);
        if (j && typeof j === 'object')
            return j;
    }
...
function cleanupChannelName(name) {
    if (!name)
        return 'Unknown';
...
function findBestLogo(countryId, baseName) {
    ...
    for (const key of Object.keys(logosMap)) {
        ...
        if (score > best) {
            best = score;
            bestUrl = logosMap[key];
        }
    }
    return best >= 0.85 ? bestUrl : undefined;
}
function findBestLogoAny(baseName) {
    const target = baseName;
    let bestUrl;
    let best = 0;
...
function categoriesOptionsForCountry(countryId) {
    try {
        const prefix = `${countryId}:`;
        const set = new Set();
...
function findBestCategory(countryId, baseName) {
...
function shouldIncludeStreamHeaders(req) {
    try {
        ...
        const cfg = m?.[1] || '';
        if (cfg.toLowerCase().split('-').includes('hdr1'))
            return true;
    }
    catch { }
    return false; // default OFF
}
const CACHE_FILE = path_1.default.join(__dirname, 'vavoo_catalog_cache.json');
let currentCache = { updatedAt: 0, countries: {} };
...
function getClientIpFromReq(req) {
    try {
        ...
        const expIp = normalize(String(req?.ip || ''));
        if (expIp)
            return expIp;
        // 3) Socket addresses
        const ra = normalize(String(req?.socket?.remoteAddress || req?.connection?.remoteAddress || ''));
        if (ra)
            return ra;
        return null;
    }
...
async function getVavooSignature(clientIp) {
    const body = {
    ...
    const headers = { 'user-agent': 'okhttp/4.11.0', 'accept': 'application/json', 'content-type': 'application/json; charset=utf-8', 'accept-encoding': 'gzip' };
    vdbg('PING ipLocation', clientIp);
    const res = await (0, node_fetch_1.default)('https://www.vavoo.tv/api/app/ping', { method: 'POST', headers, body: JSON.stringify(body), timeout: 8000 });
    if (!res.ok)
        return null;
    const json = await res.json();
    return json?.addonSig || null;
}
async function vavooCatalog(group, signature) {
    const headers = { 'user-agent': 'okhttp/4.11.0', 'accept': 'application/json', 'content-type': 'application/json; charset=utf-8', 'accept-encoding': 'gzip', 'mediahubmx-signature': signature };
    const out = [];
    let cursor = 0;
    do {
        ...
        const res = await (0, node_fetch_1.default)('https://vavoo.to/mediahubmx-catalog.json', { method: 'POST', headers, body: JSON.stringify(body), timeout: 10000 });
        if (!res.ok)
            break;
...
async function resolveVavooPlay(url, signature) {
    const headers = { 'user-agent': 'MediaHubMX/2', 'accept': 'application/json', 'content-type': 'application/json; charset=utf-8', 'accept-encoding': 'gzip', 'mediahubmx-signature': signature };
    const res = await (0, node_fetch_1.default)('https://vavoo.to/mediahubmx-resolve.json', { method: 'POST', headers, body: JSON.stringify({ language: 'de', region: 'AT', url, clientVersion: '3.1.21' }), timeout: 8000 });
    if (!res.ok)
        return null;
    const j = await res.json();
    if (Array.isArray(j) && j[0]?.url)
        return String(j[0].url);
    if (j?.url)
        return String(j.url);
    return null;
}
// Combined resolve that forwards client IP and rewrites addonSig to prioritize the observed client IP
async function resolveVavooCleanUrl(vavooPlayUrl, clientIp) {
    try {
        if (!vavooPlayUrl || !vavooPlayUrl.includes('vavoo.to'))
            return null;
        ...
        const pingHeaders = { 'user-agent': 'okhttp/4.11.0', 'accept': 'application/json', 'content-type': 'application/json; charset=utf-8', 'accept-encoding': 'gzip' };
        if (clientIp) {
        ...
        const pingRes = await (0, node_fetch_1.default)('https://www.vavoo.tv/api/app/ping', { method: 'POST', headers: pingHeaders, body: JSON.stringify(pingBody), timeout: 12000 });
        vdbg('Ping response', { status: pingRes.status, ok: pingRes.ok, tookMs: Date.now() - startedAt });
        let addonSig = null;
        if (!pingRes.ok) {
            let text = '';
            try {
                text = await pingRes.text();
            }
            catch { }
            ...
            const fallbackHeaders = { 'user-agent': 'okhttp/4.11.0', 'accept': 'application/json', 'content-type': 'application/json; charset=utf-8', 'accept-encoding': 'gzip' };
            vdbg('Ping FALLBACK without client headers/ipLocation');
            const pingRes2 = await (0, node_fetch_1.default)('https://www.vavoo.tv/api/app/ping', { method: 'POST', headers: fallbackHeaders, body: JSON.stringify(fallbackBody), timeout: 12000 });
            vdbg('Ping fallback response', { status: pingRes2.status, ok: pingRes2.ok });
            if (!pingRes2.ok)
                return null;
            const pj2 = await pingRes2.json();
        ...
        try {
            ...
            if (sigObj) {
                let dataObj = {};
                try {
                    dataObj = JSON.parse(sigObj?.data || '{}');
                }
                catch { }
                const currentIps = Array.isArray(dataObj.ips) ? dataObj.ips : [];
                vdbg('addonSig.data ips (before):', currentIps);
                if (clientIp) {
                    const newIps = [clientIp, ...currentIps.filter((x) => x && x !== clientIp)];
                    dataObj.ips = newIps;
                    if (typeof dataObj.ip === 'string')
                        dataObj.ip = clientIp;
        ...
        // Resolve
        const resolveHeaders = { 'user-agent': 'MediaHubMX/2', 'accept': 'application/json', 'content-type': 'application/json; charset=utf-8', 'accept-encoding': 'gzip', 'mediahubmx-signature': addonSig };
        if (clientIp) {
            // Minimal forwarding
        ...
        const resolveRes = await (0, node_fetch_1.default)('https://vavoo.to/mediahubmx-resolve.json', { method: 'POST', headers: resolveHeaders, body: JSON.stringify({ language: 'de', region: 'AT', url: vavooPlayUrl, clientVersion: '3.1.21' }), timeout: 12000 });
        vdbg('Resolve response', { status: resolveRes.status, ok: resolveRes.ok, tookMs: Date.now() - startedAt });
        if (!resolveRes.ok) {
            let text = '';
            try {
                text = await resolveRes.text();
            }
            catch { }
        ...
        let resolved = null;
        if (Array.isArray(resolveJson) && resolveJson.length && resolveJson[0]?.url)
            resolved = String(resolveJson[0].url);
        else if (resolveJson && typeof resolveJson === 'object' && resolveJson.url)
            resolved = String(resolveJson.url);
        if (!resolved) {
...
const epg = new service_1.EPGService({ url: epgUrl, refreshCron: '0 */3 * * *' });
...
builder.defineCatalogHandler(async ({ id, type, extra }) => {
    ...
    const items = currentCache.countries[country.id] || [];
    const selectedGenre = extra && typeof extra.genre === 'string' ? String(extra.genre) : undefined;
    // Grab EPG index once for this request
    const epgIdx = epg.getIndex();
    // First pass: compute cleaned names and counts
    const cleaned = items.map((it) => cleanupChannelName(it?.name || 'Unknown'));
    const totals = {};
    for (const n of cleaned)
        totals[n] = (totals[n] || 0) + 1;
    // Prepare array and sort with priority: SKY -> EUROSPORT -> DAZN -> A-Z
    let rows = items.map((it, idx) => ({ it, baseName: cleaned[idx] || 'Unknown' }));
    ...
    const metas = rows.map(({ it, baseName }) => {
        ...
        try {
            const key = (0, nameMap_1.normalizeChannelName)(baseName);
            ...
            for (const chId of candidates) {
                ...
                if (nn?.next && !nextTitle) {
                    nextTitle = nn.next.title;
                    nextDesc = nn.next.desc;
                }
                if (nowTitle && nextTitle)
                    break;
            }
            if (nowTitle || nowDesc || nextTitle || nextDesc) {
                const parts = [];
                if (nowTitle || nowDesc)
                    parts.push(`🔴 ${[nowTitle, short(nowDesc)].filter(Boolean).join(' — ')}`);
                if (nextTitle || nextDesc)
                    parts.push(`➡️ ${[nextTitle, short(nextDesc)].filter(Boolean).join(' — ')}`);
...
builder.defineMetaHandler(async ({ type, id }) => {
    ...
    try {
        ...
        let rest = '';
        if (id.startsWith('vavoo:'))
            rest = id.slice('vavoo:'.length);
        else if (id.startsWith('vavoo_'))
            rest = id.slice('vavoo_'.length);
        else if (id.startsWith('vavoo'))
            rest = id.slice('vavoo'.length);
        ...
        const metaOut = { id, type: 'tv', name, poster, posterShape: 'landscape', logo: poster, background: poster };
        if (nowTitle || nowDesc || nextTitle || nextDesc) {
            const parts = [];
            if (nowTitle || nowDesc)
                parts.push(`🔴 ${[nowTitle, short(nowDesc)].filter(Boolean).join(' — ')}`);
...
const lastIpByStreamId = new Map();
builder.defineStreamHandler(async ({ id }, req) => {
    try {
        ...
        let clientIp = getClientIpFromReq(req);
        if (!clientIp)
            clientIp = lastIpByStreamId.get(id)?.ip || null;
        // Cleanup entries older than 2 minutes
        const now = Date.now();
        for (const [k, v] of Array.from(lastIpByStreamId.entries())) {
            if (now - v.ts > 120000)
                lastIpByStreamId.delete(k);
        }
        ...
        const defaultHdrs = { 'User-Agent': DEFAULT_VAVOO_UA, 'Referer': 'https://vavoo.to/' };
        const hdrs = includeHdrs ? (resolved.headers || defaultHdrs) : undefined;
        const streams = [
            includeHdrs
                ? { name: 'Vavoo', title: `[🏠] ${name}`, url: resolved.url, behaviorHints: { notWebReady: true, headers: hdrs, proxyHeaders: hdrs, proxyUseFallback: true } }
                : { name: 'Vavoo', title: `[🏠] ${name}`, url: resolved.url, behaviorHints: { notWebReady: true } }
        ];
        return { streams };
    }
    catch (e) {
...
const router = (0, stremio_addon_sdk_1.getRouter)(builder.getInterface());
...
app.use(express_1.default.static(__dirname, { etag: false, maxAge: 0 }));
// Compute absolute URL for fallback poster/logo on each request
const FALLBACK_POSTER_FILE = path_1.default.join(__dirname, 'tvvoo.png');
...
app.get('/cfg-:cfg/manifest.json', (req, res) => {
    try {
        ...
        const dyn = {
            ...
            catalogs: countries.map(c => {
                ...
                const extra = opts.length ? [{ name: 'genre', options: opts, isRequired: false }] : [];
...
app.get('/configure/:cfg/manifest.json', (req, res, next) => {
    // Re-route to '/:cfg/manifest.json'
    req.url = `/${encodeURIComponent(String(req.params.cfg || ''))}/manifest.json`;
    next();
});
...
app.use('/catalog', (_req, res, next) => {
    res.setHeader('Cache-Control', 'no-store');
    next();
});
app.use('/stream', (_req, res, next) => {
    res.setHeader('Cache-Control', 'no-store');
    next();
});
// Prefixed routes (path-based config)
app.use('/:cfg/catalog', (_req, res, next) => {
    res.setHeader('Cache-Control', 'no-store');
    next();
});
app.use('/:cfg/stream', (_req, res, next) => {
    res.setHeader('Cache-Control', 'no-store');
    next();
});
app.use('/cfg-:cfg/catalog', (_req, res, next) => {
    res.setHeader('Cache-Control', 'no-store');
    next();
});
app.use('/cfg-:cfg/stream', (_req, res, next) => {
    res.setHeader('Cache-Control', 'no-store');
    next();
});
...
app.get('/', (_req, res) => {
    ...
    try {
        const filePath = path_1.default.join(__dirname, 'landing.html');
...
app.get('/cfg-:cfg/configure', (_req, res) => {
    res.setHeader('content-type', 'text/html; charset=utf-8');
    try {
        const filePath = path_1.default.join(__dirname, 'landing.html');
        const html = fs_1.default.readFileSync(filePath, 'utf8');
...
app.get('/tvvoo.png', (_req, res) => {
    const candidates = [
        path_1.default.join(__dirname, 'tvvoo.png'),
        path_1.default.resolve(__dirname, '../public/tvvoo.png'),
    ];
    for (const p of candidates) {
        try {
            if (fs_1.default.existsSync(p)) {
                const bin = fs_1.default.readFileSync(p);
...
app.get('/debug/resolve', async (req, res) => {
    try {
        ...
        if (!sig)
            return res.status(502).json({ error: 'no-signature' });
        const resolved = await resolveVavooPlay(url, sig);
        if (!resolved)
            return res.status(404).json({ error: 'not-resolved' });
        const hdrs = { 'User-Agent': DEFAULT_VAVOO_UA, 'Referer': 'https://vavoo.to/' };
...
app.get('/epg/lookup', (req, res) => {
    try {
        const name = String(req.query.name || '');
        const idx = epg.getIndex();
        const key = (0, nameMap_1.normalizeChannelName)(name);
        const candidates = idx.nameToIds?.[key] || [];
...
app.use((req, _res, next) => {
    if (process.env.VAVOO_DEBUG !== '0') {
        try {
            console.log('[VAVOO] HTTP', req.method, req.url);
        }
        catch { }
    }
    next();
...
function countryIdToTvLogosDir(id) {
    const map = {
        it: 'italy',
        fr: 'france',
        de: 'germany',
...
async function updateLogosFromGitHub() {
    try {
        let added = 0;
        for (const c of SUPPORTED_COUNTRIES) {
            ...
            const resp = await (0, node_fetch_1.default)(apiUrl, { headers: { 'user-agent': 'TvVoo/1.0', 'accept': 'application/vnd.github.v3+json' } });
            if (!resp.ok)
                continue;
        ...
        if (added > 0) {
            try {
                fs_1.default.writeFileSync(LOGOS_FILE, JSON.stringify(logosMap, null, 2), 'utf8');
            }
            catch { }
        }
        if (added)
            vdbg('Logos map updated from GitHub with', added, 'entries');
        return added;
    }
    catch (e) {
...
async function updateLogosFromM3U() {
    try {
        const url = 'https://raw.githubusercontent.com/nzo66/TV/main/lista.m3u';
        const resp = await (0, node_fetch_1.default)(url, { timeout: 8000 });
        if (!resp.ok)
            return 0;
        ...
        if (catsAdded > 0) {
            try {
                fs_1.default.writeFileSync(CATEGORIES_FILE, JSON.stringify(categoriesMap, null, 2), 'utf8');
            }
            catch { }
        ...
        for (const c of SUPPORTED_COUNTRIES) {
            try {
                ...
                if (!items || items.length === 0) {
                    try {
                        items = await vavooCatalog(c.group, sig);
                    }
                    catch { }
                }
...
node_cron_1.default.schedule('0 2 * * *', () => { refreshDailyCache().catch(() => { }); }, { timezone: 'Europe/Rome' });
