...
// Helper to compute countries count from cfg path formats
function countCountriesFromCfgParam(rawCfg: string): number {
  try {
    if (!rawCfg) return 0;
    // Two formats supported:
    // 1) dash format: it-uk-fr-ex-de (we only care includes)
    if (rawCfg.includes('-') && !rawCfg.includes('=')) {
      const [incPart] = rawCfg.toLowerCase().split('-ex-');
      const incList = (incPart || '').split('-').map(s => s.trim()).filter(Boolean);
      const valid = new Set(SUPPORTED_COUNTRIES.map(c => c.id));
      return incList.filter(id => valid.has(id)).length;
    }
    // 2) key=value&... format: include=it,uk,fr&exclude=de
    const parts = rawCfg.split('&').filter(Boolean);
    const cfg: Record<string, string> = {};
    for (const p of parts) {
      const [k, v] = p.split('=');
      if (!k) continue;
    ...
    const include = cfg.include ? cfg.include.split(',').map(s => s.trim()) : [];
    const valid = new Set(SUPPORTED_COUNTRIES.map(c => c.id));
    return include.filter(id => valid.has(id)).length;
  } catch { return 0; }
}

// Enforce empty results for over-limit configurations on cfg-prefixed resource routes
function guardOverLimitCatalog(req: Request, res: Response, next: NextFunction) {
  res.setHeader('Cache-Control', 'no-store');
  try {
    const cfg = String((req.params as any).cfg || '');
    const count = countCountriesFromCfgParam(cfg);
    if (count > 3) return res.json({ metas: [] });
  } catch {}
  next();
}
function guardOverLimitMeta(req: Request, res: Response, next: NextFunction) {
  res.setHeader('Cache-Control', 'no-store');
  try {
    const cfg = String((req.params as any).cfg || '');
    const count = countCountriesFromCfgParam(cfg);
    if (count > 3) return res.json({ meta: null });
  } catch {}
  next();
}
function guardOverLimitStream(req: Request, res: Response, next: NextFunction) {
  res.setHeader('Cache-Control', 'no-store');
  try {
    const cfg = String((req.params as any).cfg || '');
    const count = countCountriesFromCfgParam(cfg);
    if (count > 3) return res.json({ streams: [] });
  } catch {}
  next();
}
app.use('/:cfg/catalog', guardOverLimitCatalog);
app.use('/:cfg/meta', guardOverLimitMeta);
app.use('/:cfg/stream', guardOverLimitStream);
app.use('/cfg-:cfg/catalog', guardOverLimitCatalog);
app.use('/cfg-:cfg/meta', guardOverLimitMeta);
app.use('/cfg-:cfg/stream', guardOverLimitStream);
